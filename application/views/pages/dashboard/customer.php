...
$unread_notif = $this->db->where('deleted_at is null')
	->where('is_readed', 0)
	->group_start()
	->where('to_user', $this->session->userdata('user')['id_user'])
	->or_where('to_role', $this->session->userdata('user')['id_usr_role'])
	->group_end()
...
$filter_total_vendor_verification="";
if($this->session->userdata('user')['id_usr_role']==3){
	$filter_total_vendor_verification=" and company.id_company_owner =".$this->session->userdata('user')['id_company_owner'];
}
				...
				<div class="no-gutters row">
					...
					<div class="col-sm-6" id="to_vendor">
						<div class="widget-chart widget-chart-hover br-br">
							...
							<div class="widget-numbers" class="procurement"><?php echo count($total_vendor_aktif) ?></div>
				...
				<div class="row">
					...
					<div class="col-md-6">
						...
						<select class="form-control" id="fmost_trx_branch">
							...
							$currentBranch = $this->session->userdata('user')['branch_code'];
							if ($currentBranch == 1) {
								echo '<option value="" selected>Seluruh Divre</option>';
							}
							foreach ($branchCode as $b) {
								if ($b->id == $currentBranch || $currentBranch == 1) {
...
<script>
	$(document).ready(function() {
		...
		let fMostTable = $("#fmost_table").DataTable({
			'ajax': {
				"type": "POST",
		...
		let fmostLengtText = $('.fmost_trx_length_txt');
